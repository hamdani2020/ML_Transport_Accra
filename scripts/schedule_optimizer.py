...
class ScheduleOptimizer:
    ...
    def prepare_schedule_data(self, data: Dict[str, pd.DataFrame]) -> Dict:
        ...
        for route_id in unique_routes[:5]:  # Limit to first 5 routes for demonstration
            ...
            # Demand patterns are now injected into schedule_data before optimization.
    ...
    def optimize_schedules(self, schedule_data: Dict) -> Dict:
        ...
        routes = list(schedule_data['route_info'].keys())
        time_periods = ['morning_peak', 'midday', 'afternoon_peak', 'evening', 'night']
...
def main():
    ...
    # When running standalone, generate demand patterns.
    # In a pipeline, this would be injected from a demand prediction step.
    logger.info("Generating demand patterns for standalone run.")
    for route_id in schedule_data['route_info']:
        total_stops = schedule_data['route_info'][route_id]['total_stops']
        schedule_data['demand_patterns'][route_id] = optimizer._generate_demand_pattern(route_id, total_stops)
